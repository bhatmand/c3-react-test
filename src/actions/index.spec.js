...
describe("Actions", () => {
  ...
  it("getUsers - fetch ajax content --- error", async () => {
    ...
    await getUsers()(dispatch);
    expect(dispatch.mock.calls[0][0]).toEqual(output);
    // TODO: this does not work... needs fix
    // expect(dispatch.mock.calls[1][0]).toEqual(output1);
  });

  it("deleteUser - returns updated payload with provided user id removed", () => {
    const input = {
      Users: {
        users: [
          {
  ...
  it("removeDuplicates - returns updated payload with duplicate user id(s) removed", () => {
    const input = {
      Users: {
        users: [
          {
